...
static const int wCRCTable[] = { 0X0000, 0XC0C1, 0XC181, 0X0140, 0XC301, 0X03C0,
...
int main(int argc, char **argv) {
	...
	for (cont = 0; cont < sizeof(wCRCTable) / sizeof(int); ant =
			wCRCTable[cont], ++cont)
		printf("wCRCTable[%03d] = 0x%X\t%6d\t%6.3f\t%d\t%d\n", cont,
				wCRCTable[cont], wCRCTable[cont] - ant,
				(double) wCRCTable[cont] / ant, wCRCTable[cont], ant);

	printf("Calculados %d posicoes com %d no array sendo %d o tamanho do int",
			cont, (int) sizeof(wCRCTable), (int) sizeof(int));
